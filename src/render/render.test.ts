...
describe('render', () => {
  describe('document shell', () => {
    ...
    it('inlines a base stylesheet in the head', async () => {
      expect((await render('# Hello')).html).toMatch(/<style>[\s\S]+<\/style>/);
    });

    it('derives the document title from the first H1', async () => {
      expect((await render('# My Document\n\nbody')).html).toMatch(/<title>My Document<\/title>/);
    });

    it('falls back to a default title when there is no H1', async () => {
      expect((await render('just text')).html).toMatch(/<title>open-md<\/title>/);
    });

    it('returns a non-negative parseMs', async () => {
      const { parseMs } = await render('# Hello');
  ...
  describe('core GFM', () => {
    ...
    it('renders footnotes', async () => {
      expect((await render('Here[^1]\n\n[^1]: the note')).html).toContain('footnote');
    });

    it('renders strikethrough', async () => {
      expect((await render('~~gone~~')).html).toContain('<s>gone</s>');
    });

    it('autolinks bare URLs', async () => {
      expect((await render('see https://example.com here')).html).toContain(
  ...
  describe('math (KaTeX) assets', () => {
    ...
    it('injects KaTeX for display math', async () => {
      expect((await render('$$\\int_0^1 x\\,dx$$')).html).toContain('renderMathInElement');
    });
  ...
  describe('trusted HTML passthrough', () => {
    it('allows intentional inline HTML through', async () => {
      expect((await render('<div class="note">hi</div>')).html).toContain(
